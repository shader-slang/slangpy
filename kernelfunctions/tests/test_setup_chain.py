...
def test_build_chain_function_only(device_type: DeviceType):
    ...
    chain = function._build_call_data(1, 1).chain
    assert len(chain) == 1
    assert isinstance(chain[0], kf.Function)


@pytest.mark.parametrize("device_type", helpers.DEFAULT_DEVICE_TYPES)
def test_build_chain_with_sets(device_type: DeviceType):
    ...
    func_call = function._build_call_data(1, 1)
    ...
    s1_call = s1._build_call_data(1, 1)
    ...
    s2_call = s2._build_call_data(1, 1)
...
@pytest.mark.parametrize("device_type", helpers.DEFAULT_DEVICE_TYPES)
def test_set_arguments(device_type: DeviceType):
...
@pytest.mark.parametrize("device_type", helpers.DEFAULT_DEVICE_TYPES)
def test_set_lambda_callback(device_type: DeviceType):
    ...
    call_data = chain._build_call_data(1, 1)
...
@pytest.mark.parametrize("device_type", helpers.DEFAULT_DEVICE_TYPES)
def test_set_instance_callback(device_type: DeviceType):
