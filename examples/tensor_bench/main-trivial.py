...
def run(
    ...
    program = device.load_program("test-trivial.slang", entry_point_names=["compute_main"])
    ...
    avg_time = np.median(deltas)
    print(f"Median time: {avg_time:.3f} ms")
...
for name, device_type in BACKENDS.items():
    ...
    results[name] = {}
    for input_tensor_count in [1, 2, 4, 8, 16, 32, 64]:
        time = run(
            device_type,
            input_tensor_count,
            "INDEX_MODE_VECTOR",
