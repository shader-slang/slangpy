...
def calcCompressedDispatchIDs(x_max : int, y_max : int, wg_x : int, wg_y : int):
    ...
    local_xyv = np.stack([local_yv, local_xv], axis=-1)
    ...
    group_xyv = np.stack([group_yv, group_xv], axis=-1)
    group_xyv = np.tile(group_xyv[:, :, None, None, :], (1, 1, wg_x, wg_y, 1))
...
dispatch_ids.from_numpy(calcCompressedDispatchIDs(W, H, WORKGROUP_X, WORKGROUP_Y))
