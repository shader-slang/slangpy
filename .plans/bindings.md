...
- Renamed the Python marshall API to `WriteToCursorMarshall`, `WriteToCursorMarshallInfo`, and `register_cursor_writer_marshal`.
...
- Rename `slangpy/bindings/cursor.py` classes and APIs:
  ...
  - `register_cursor_type` -> `register_cursor_writer_marshal`, if keeping a Python-side registration API for Python-only future work
...
```python
register_cursor_writer_marshal(TextureHandle)
```
