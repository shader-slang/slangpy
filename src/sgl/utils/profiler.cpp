...
namespace {
    ...
    constexpr uint32_t kGpuQueryTlsCacheSize = 16;
    ...
    struct ThreadData {
        ...
        explicit ThreadData(Profiler* profiler_)
            : profiler(profiler_)
        {
        }
    ...
    struct StatsSampleRing {
        ...
        void set_capacity(uint32_t capacity)
        {
            ...
            retained.reserve(std::min<uint32_t>(sample_count, capacity));
            const uint32_t first = sample_count > capacity ? sample_count - capacity : 0;
            for (uint32_t i = first; i < sample_count; ++i)
                retained.push_back(sample(i));

            samples.assign(capacity, 0.0);
            sample_count = uint32_t(retained.size());
            next_index = sample_count % std::max<uint32_t>(capacity, 1);
            for (uint32_t i = 0; i < sample_count; ++i)
                samples[i] = retained[i];
        }

        void push(double value_ms, uint32_t capacity)
        {
            set_capacity(capacity);
            if (samples.empty())
                return;

            samples[next_index] = value_ms;
            next_index = (next_index + 1) % uint32_t(samples.size());
            sample_count = std::min<uint32_t>(sample_count + 1, uint32_t(samples.size()));
        }

        double sample(uint32_t index) const
        {
            SGL_ASSERT(index < sample_count);
            if (sample_count < samples.size())
                return samples[index];
            const uint32_t begin = next_index % uint32_t(samples.size());
            return samples[(begin + index) % uint32_t(samples.size())];
        }
    };
    ...
    struct StatsNodeState {
        ...
        StatsSampleRing cpu_samples;
        StatsSampleRing gpu_samples;
    };
    ...
    struct GpuQuerySlot {
        ...
        uint64_t command_stream_id{0};
        bool closed{false};
        bool recording_submitted{false};
        bool recording_discarded{false};
        bool pending_emitted{false};
        bool completed{false};
    };

    struct GpuRecordingLifecycleEvent {
        Device* device{nullptr};
        CommandRecordingID command_stream_id{0};
    ...
    struct GpuContext {
        ...
        CommandQueueType queue{CommandQueueType::graphics};
        uint32_t timeline_id{kInvalidProfilerId};
        DeviceCallbackID submitted_callback_id{0};
        DeviceCallbackID discarded_callback_id{0};
        bool disabled{false};
        uint64_t generation{1};
        ref<QueryPool> query_pool;
        std::vector<GpuQuerySlot> slots;
        std::vector<GpuQuerySlot*> free_slots;
    };
...
struct ProfilerImpl {
    ...
    moodycamel::ConcurrentQueue<GpuRecordingLifecycleEvent> gpu_recording_lifecycle_queue;
    ...
    ThreadData* get_or_create_thread_data(std::thread::id thread_id)
    {
        ...
        ThreadData* thread_data = new ThreadData(profiler);
        {
            ...
            trace_timelines.push_back({
                ...
                .type = ProfilerTimelineType::cpu,
                .name = fmt::format("CPU Thread {}", thread_hash),
    ...
    bool range_in_trace_window_locked(uint64_t begin_timestamp, uint64_t end_timestamp) const
    {
        ...
        if (begin_timestamp < trace_epoch_start_timestamp || begin_timestamp < trace_start_timestamp)
            return false;
        if (trace_stop_timestamp != 0)
            return end_timestamp <= trace_stop_timestamp;
        return trace_active;
    }
    ...
    {
        if (!zone.gpu.ticket || zone.gpu.timeline_id == kInvalidProfilerId)
            return;
    ...
    void process_cpu_end_zone(ThreadData& thread_data, const ProfilerEvent& event)
    {
        ...
        std::vector<uint32_t> completed_trace_roots;
        if (range_in_trace_window_locked(zone.start_timestamp, end_timestamp)) {
            const uint32_t trace_zone_id = uint32_t(trace_zones.size());
            ...
            ProfilerZoneRecord& trace_zone = trace_zones.back();
            append_trace_child_range_locked(trace_zone, zone.trace_children);
            completed_trace_roots.push_back(trace_zone_id);
        } else {
            completed_trace_roots = std::move(zone.trace_children);
        }

        if (!completed_trace_roots.empty()) {
            if (!thread_data.zone_stack.empty()) {
                std::vector<uint32_t>& parent_children = thread_data.zone_stack.back().trace_children;
                parent_children
                    .insert(parent_children.end(), completed_trace_roots.begin(), completed_trace_roots.end());
            } else {
                trace_root_indices
                    .insert(trace_root_indices.end(), completed_trace_roots.begin(), completed_trace_roots.end());
            }
        }

        if (zone.stats_node_id != kInvalidProfilerId && zone.stats_node_id < stats_nodes.size()) {
            const double duration_ms = double(end_timestamp - zone.start_timestamp) / 1000000.0;
            stats_nodes[zone.stats_node_id].cpu_samples.push(duration_ms, profiler->stats_window_size());
        }
    ...
    void process_begin_frame(ThreadData& thread_data, const ProfilerEvent& event)
    {
        ...
        if (thread_data.active_frame.frame_id != kInvalidProfilerId) {
    ...
    void process_end_frame(ThreadData& thread_data, const ProfilerEvent& event)
    {
        ...
        OpenFrame frame = thread_data.active_frame;
        thread_data.active_frame = {};
        const uint64_t end_timestamp = std::max(frame.start_timestamp, event.timestamp);

        if (range_in_trace_window_locked(frame.start_timestamp, end_timestamp)) {
    ...
    uint32_t try_complete_gpu_zone_locked(ThreadData& thread_data, uint32_t zone_index)
    {
        ...
        uint32_t trace_zone_id = kInvalidProfilerId;
        if (range_in_trace_window_locked(record.start_timestamp, record.end_timestamp)) {
        ...
        } else if (record.parent_index < 0 && !record.parent_pending) {
            trace_root_indices.insert(trace_root_indices.end(), child_zone_ids.begin(), child_zone_ids.end());
        }

        if (record.stats_node_id != kInvalidProfilerId && record.stats_node_id < stats_nodes.size()) {
            stats_nodes[record.stats_node_id].gpu_samples.push(
                double(record.end_timestamp - record.start_timestamp) / 1000000.0,
                profiler->stats_window_size()
            );
        }
    ...
    void store_orphan_gpu_query_result_locked(ThreadData& thread_data, const ProfilerGpuQueryResultEvent& result)
    {
        ...
        if (result.state == QueryResultState::resolved || result.state == QueryResultState::reset
            || orphan.state != QueryResultState::resolved) {
            orphan = result;
        }
    }
    ...
    void apply_gpu_query_result_locked(ThreadData& thread_data, const ProfilerGpuQueryResultEvent& result)
    {
        ...
        switch (result.state) {
        case QueryResultState::pending:
            mark_gpu_zone_pending_locked(record);
            break;
        case QueryResultState::reset:
            drop_gpu_zone_locked(thread_data, zone_index);
            break;
        case QueryResultState::resolved:
    ...
    void set_stats_window_size(uint32_t size)
    {
        ...
        for (StatsNodeState& node : stats_nodes) {
            node.cpu_samples.set_capacity(size);
            node.gpu_samples.set_capacity(size);
        }
        publish_stats_snapshot_locked();
    }
    ...
    ProfilerStatValue summarize_samples_locked(const StatsSampleRing& samples) const
    {
        ...
        double sum = 0.0;
        double sum_sq = 0.0;
        for (uint32_t i = 0; i < samples.sample_count; ++i) {
            const double sample = samples.sample(i);
            if (!value.valid) {
                value.min_ms = sample;
                value.max_ms = sample;
                value.valid = true;
            } else {
                value.min_ms = std::min(value.min_ms, sample);
                value.max_ms = std::max(value.max_ms, sample);
            }
            value.last_ms = sample;
            sum += sample;
    ...
    ref<ProfilerStats> build_stats_snapshot_locked()
    {
    ...
    void publish_stats_snapshot_locked()
    {
        ref<ProfilerStats> stats = build_stats_snapshot_locked();
        std::lock_guard publish_lock(published_stats_mutex);
        published_stats = std::move(stats);
    }

    ref<ProfilerStats> stats_snapshot()
    {
        std::lock_guard lock(published_stats_mutex);
        return published_stats ? published_stats : make_ref<ProfilerStats>();
    }
    ...
    GpuContext& get_or_create_gpu_context_locked(Device* device, CommandQueueType queue)
    {
        ...
        context.queue = queue;
        const std::string timeline_name = fmt::format("GPU {} {}", device->info().adapter_name, enum_to_string(queue));
        const uint64_t timeline_device_id = uint64_t(reinterpret_cast<uintptr_t>(device));
        context.submitted_callback_id = device->register_command_recording_submitted_callback(
            [this](const CommandRecordingSubmittedEvent& event)
            {
                queue_gpu_recording_lifecycle_event(event.device, event.id, true);
            }
        );
        context.discarded_callback_id = device->register_command_recording_discarded_callback(
            [this](const CommandRecordingDiscardedEvent& event)
            {
                queue_gpu_recording_lifecycle_event(event.device, event.id, false);
            }
        );
        {
            std::lock_guard data_lock(data_mutex);
            ...
            trace_timelines.push_back({
                ...
                .type = ProfilerTimelineType::gpu,
                .name = timeline_name,
                .device_id = timeline_device_id,
                .queue = queue,
            });
        }
        ...
        SGL_ASSERT(inserted);
        GpuContext& inserted_context = inserted_it->second;

        try {
            const uint32_t pool_size = profiler->desc().gpu_query_pool_size;
            inserted_context.query_pool = inserted_context.device->create_query_pool({
                .type = QueryType::timestamp,
                .count = pool_size,
            });
            inserted_context.slots.reserve(pool_size / 2);
            inserted_context.free_slots.reserve(pool_size / 2);
            for (uint32_t query_index = 0; query_index + 1 < pool_size; query_index += 2) {
                inserted_context.slots.push_back({
                    .query_pool = inserted_context.query_pool,
                    .begin_query_index = query_index,
                    .end_query_index = query_index + 1,
                    .queue = inserted_context.queue,
                    .context = &inserted_context,
                    .completed = true,
                });
            }
            for (GpuQuerySlot& slot : inserted_context.slots)
                inserted_context.free_slots.push_back(&slot);
        } catch (...) {
            inserted_context.disabled = true;
            log_warn_once(
                "Profiler failed to create GPU timestamp query pool; GPU profiling is disabled for that context."
            );
        }
    ...
    void unregister_gpu_callbacks() noexcept
    {
        std::lock_guard lock(gpu_mutex);
        for (auto& [key, context] : gpu_contexts) {
            (void)key;
            if (!context.device)
                continue;
            if (context.submitted_callback_id != 0) {
                context.device->unregister_command_recording_submitted_callback(context.submitted_callback_id);
                context.submitted_callback_id = 0;
            }
            if (context.discarded_callback_id != 0) {
                context.device->unregister_command_recording_discarded_callback(context.discarded_callback_id);
                context.discarded_callback_id = 0;
            }
        }
    }

    void
    queue_gpu_recording_lifecycle_event(Device* device, CommandRecordingID command_stream_id, bool submitted) noexcept
    {
        if (!gpu_recording_lifecycle_queue.enqueue({
                .device = device,
    ...
    enum class GpuQueryRecordingState {
        awaiting_submit,
        submitted,
        discarded,
    };

    GpuQueryRecordingState gpu_query_recording_state(GpuQuerySlot* slot) noexcept
    {
        std::lock_guard lock(gpu_mutex);
        if (slot->recording_discarded)
            return GpuQueryRecordingState::discarded;
        if (slot->recording_submitted)
            return GpuQueryRecordingState::submitted;
        return GpuQueryRecordingState::awaiting_submit;
    }
    ...
    void drain_gpu_recording_lifecycle_events()
    {
        GpuRecordingLifecycleEvent event;
        std::lock_guard lock(gpu_mutex);
        while (gpu_recording_lifecycle_queue.try_dequeue(event)) {
            for (auto& [key, context] : gpu_contexts) {
                (void)key;
                if (context.device.get() != event.device)
                    continue;
                for (GpuQuerySlot& slot : context.slots) {
                    if (slot.completed || slot.command_stream_id != event.command_stream_id)
                        continue;
                    if (event.submitted)
                        slot.recording_submitted = true;
                    else
                        slot.recording_discarded = true;
                }
            }
        }
    }
    ...
    ) noexcept
    {
        ...
        if (cache.profiler != this || !cache.context || cache.free_slots.empty())
            return {};
        if (cache.generation != cache.context->generation)
            return {};
        ...
        GpuQuerySlot* slot = cache.free_slots.back();
        cache.free_slots.pop_back();
        slot->ticket = {.id = next_gpu_query_ticket.fetch_add(1, std::memory_order_acq_rel)};
        slot->owner_thread = owner_thread;
        slot->command_stream_id = command_stream_id;
        slot->closed = false;
        slot->recording_submitted = false;
        slot->recording_discarded = false;
    ...
    void refill_gpu_query_cache_locked(GpuContext& context)
    {
        s_gpu_query_cache = {
            .profiler = this,
            .context = &context,
        ...
        const size_t refill_count = std::min<size_t>(kGpuQueryTlsCacheSize, context.free_slots.size());
        s_gpu_query_cache.free_slots.reserve(refill_count);
        for (size_t i = 0; i < refill_count; ++i) {
            s_gpu_query_cache.free_slots.push_back(context.free_slots.back());
    ...
    ProfilerGpuQueryPair allocate_gpu_query_pair_slow(CommandEncoder* encoder, ThreadData* owner_thread)
    {
        ...
        if (context.disabled || context.free_slots.empty())
            return {};

        refill_gpu_query_cache_locked(context);
    ...
    void recycle_gpu_query_slot(GpuQuerySlot* slot) noexcept
    {
        if (!slot || !slot->context)
            return;

        std::lock_guard lock(gpu_mutex);
        GpuContext* context = slot->context;
        if (context->disabled || !slot->query_pool || !context->device || context->device->is_closed())
            return;

        try {
            slot->query_pool->reset(slot->begin_query_index, 2);
        } catch (...) {
            context->disabled = true;
            ++context->generation;
            clear_gpu_query_cache(context);
            log_warn_once(
                "Profiler failed to reset a GPU timestamp query slot; GPU profiling is disabled for that context."
            );
            return;
        }

        slot->ticket = {};
        slot->owner_thread = nullptr;
        slot->command_stream_id = 0;
        slot->closed = false;
        slot->recording_submitted = false;
        slot->recording_discarded = false;
        slot->pending_emitted = false;
        slot->completed = true;
        context->free_slots.push_back(slot);
    }
    ...
    bool process_gpu_query_slot(GpuQuerySlot* slot, std::map<GpuContextKey, GpuTimestampAnchor>& anchors)
    {
        ...
        if (!slot->query_pool || !slot->owner_thread || slot->query_pool->device()->is_closed()) {
            queue_gpu_query_result(slot, QueryResultState::reset);
            slot->completed = true;
            return true;
        }

        switch (gpu_query_recording_state(slot)) {
        case GpuQueryRecordingState::awaiting_submit:
            return false;
        case GpuQueryRecordingState::discarded:
            queue_gpu_query_result(slot, QueryResultState::reset);
            slot->completed = true;
            return true;
        case GpuQueryRecordingState::submitted:
            break;
        }

        QueryResultState state = QueryResultState::reset;
        try {
            state = slot->query_pool->get_result_state(slot->begin_query_index, 2);
        } catch (...) {
            state = QueryResultState::reset;
        }
        ...
        case QueryResultState::pending:
            if (!slot->pending_emitted) {
                queue_gpu_query_result(slot, QueryResultState::pending);
        ...
        queue_gpu_query_result(slot, QueryResultState::resolved, start_timestamp, end_timestamp);
        slot->completed = true;
        return true;
    }
    ...
    void process_gpu_query_slots()
    {
        ...
        drain_gpu_recording_lifecycle_events();
        ...
        pending_gpu_query_slots.erase(
            std::remove_if(
                ...
                {
                    const bool completed = process_gpu_query_slot(slot, anchors);
                    if (completed)
                        recycle_gpu_query_slot(slot);
                    return completed;
                }
            ),
            pending_gpu_query_slots.end()
...
namespace {
    ...
    thread_local std::vector<Profiler*> s_current_profiler_stack;
...
{
    return fmt::format(
        "Profiler(enabled = {}, auto_zones_enabled = {}, debug_groups_enabled = {}, stats_window_size = {})",
        m_enabled.load(),
...
{
    return s_current_profiler_stack.empty() ? nullptr : s_current_profiler_stack.back();
}
