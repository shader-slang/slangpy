...
void Tensor::clear(CommandEncoder* cmd)
{
    ...
    // unordered_access flag (it removes the device). For device-local read-only storage,
    // zero by uploading a host-side zero-filled copy instead (copy path, no UAV needed),
    // routing through the caller's encoder when one is supplied so ordering is preserved.
    // Only device-local buffers auto-gain copy_destination usage (see Buffer ctor), so the
    // copy path is guaranteed valid there; other memory types keep the clear_buffer path.
    if (!is_set(m_storage->desc().usage, BufferUsage::unordered_access)
        && m_storage->memory_type() == MemoryType::device_local) {
        std::vector<uint8_t> zeros(m_storage->size(), 0);
        if (cmd) {
            cmd->upload_buffer_data(m_storage, 0, zeros.size(), zeros.data());
        } else {
