...
struct SlangCompilerOptions {
    ...
    /// Inputs are assumptions, not an absolute capability ceiling.
    std::optional<std::vector<std::string>> capabilities;

    /// Adds (true) or removes (false) capability inputs, without subtracting implied/profile requirements.
    std::map<std::string, bool> capability_overrides;
...
/// Snapshot of target selection. Capabilities are forwarded inputs, not Slang's full implication closure.
struct SlangTargetInfo {
    ...
    std::vector<std::string> ignored_capabilities;
    /// Arguments generated by SlangPy (excludes arguments generated internally by Slang).
    std::vector<std::string> generated_downstream_args;
    /// Compatibility decisions and limits of validation.
    std::vector<std::string> notes;
    /// Digest of the resolved Slang session descriptor, before cache search paths are added.
...
struct SlangSessionData : Object {
    ...
    /// Target selection associated with this successfully built session.
...
public:
    ...
    SlangTargetInfo target_info() const { return m_data->target_info; }
