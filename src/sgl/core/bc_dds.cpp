...
BCCompressedImage bc_compressed_image_from_dds(const DDSFile& dds)
{
    ...
    for (uint32_t mip = 0; mip < dds.mip_count(); ++mip) {
        uint32_t mip_width = mip >= 32 ? 1u : std::max(1u, dds.width() >> mip);
        uint32_t mip_height = mip >= 32 ? 1u : std::max(1u, dds.height() >> mip);
        ...
        std::memcpy(mip_level.data.data(), dds.get_subresource_data(mip, 0), slice_pitch);
    }
...
void bc_compressed_image_to_dds(const BCCompressedImage& image, Stream* stream)
{
    ...
    uint32_t expected_width = image.mip_levels[0].width;
    uint32_t expected_height = image.mip_levels[0].height;
    SGL_CHECK(
        expected_width > 0 && expected_height > 0,
        "bc_compressed_image_to_dds: mip level 0 dimensions must be non-zero"
    );
    for (size_t i = 0; i < image.mip_levels.size(); ++i) {
        const BCCompressedMip& mip = image.mip_levels[i];
        SGL_CHECK(
            mip.width == expected_width && mip.height == expected_height,
            "bc_compressed_image_to_dds: invalid dimensions for mip level {}",
            i
        );
        SGL_CHECK(
            mip.data.size() == bc_compressed_size(mip.width, mip.height, image.format),
            "bc_compressed_image_to_dds: invalid data size for mip level {}",
            i
        );
    ...
    SGL_CHECK(
        image.mip_levels.size() <= std::numeric_limits<uint32_t>::max(),
        "bc_compressed_image_to_dds: too many mip levels"
    );
    ...
    for (const auto& mip : image.mip_levels) {
        SGL_CHECK(
            mip.data.size() <= std::numeric_limits<size_t>::max() - total_size,
            "bc_compressed_image_to_dds: total data size overflow"
        );
