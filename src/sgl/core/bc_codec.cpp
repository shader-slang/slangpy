...
struct BCCodecSWImpl : BCCodecImpl {
    ...
    BCCompressedImage encode(const BCImage& src, BCFormat format, const BCEncodeOptions& options) override
    {
        ...
        if (is_snorm(format))
            SGL_THROW("BCCodecSW::encode: BC4/BC5 SNORM encoding is not implemented");
        ...
        if (options.generate_mipmaps && (src.width > 1 || src.height > 1)) {
            ...
            // Explicitly describe the source color space instead of relying on Bitmap's integer default.
            ...
            size_t src_row_size
                = static_cast<size_t>(src.width) * src.channel_count * component_byte_size(src.component_type);
            for (uint32_t y = 0; y < src.height; ++y) {
                std::memcpy(
                    src_bmp->uint8_data() + y * src_row_size,
                    static_cast<const uint8_t*>(src.data) + static_cast<size_t>(y) * src.row_pitch,
                    src_row_size
                );
            }

            // Resample in linear float32. For sRGB formats this conversion linearizes the source.
            ref<Bitmap> float_bmp = src_bmp->convert(pf, Bitmap::ComponentType::float32, false);
            ...
            while (mip_w > 1 || mip_h > 1) {
                ...
                // BC sRGB blocks store gamma-encoded values; other formats store the linear values directly.
                ref<Bitmap> enc_bmp = srgb ? current->convert(pf, Bitmap::ComponentType::float32, true) : current;
...
struct BCCodecNVTTImpl : BCCodecImpl {
    ...
    {
        if (is_snorm(format))
            SGL_THROW("BCCodecNVTT::encode: BC4/BC5 SNORM encoding is not implemented");
...
)
{
    ...
    SGL_CHECK(
        dst.channel_count >= 1 && dst.channel_count <= 4,
        "BCCodec::decode: destination channel_count must be 1-4"
    );
    ...
    SGL_CHECK(
        ...
        "BCCodec::decode: destination row_pitch is too small"
    );
