...
- ``InstanceTensor``: Represents a list of instances of a Slang type stored in a single ``Tensor``.
- ``InstanceList``: Represents a list of instances of a Slang type stored in SOA (Structure of Arrays) form, where separate fields can be stored in separate tensors.
...
Now, let's create a tensor of 10 particles and call their constructor:

.. code-block:: python

    # ... device/module initialization here ...

    # Create a tensor of particles (.as_struct ensures proper Python typing)
    particles = spy.InstanceTensor(
...
The ``this`` parameter is implicitly passed as a **1D tensor of 10 particles**, resulting in a **1D dispatch** of 10 threads.
...
The ``update`` method is marked ``[mutating]`` in Slang, signaling to SlangPy that the ``this`` parameter is ``inout``. This ensures the modified particles are written back to the tensor after the method completes.

*Note:* Just like ``Tensor``, an ``InstanceTensor`` can be passed as a parameter to a function, and SlangPy will automatically generate the correct kernel for reading from it.

Instance Lists
--------------

``InstanceList`` functions similarly to ``InstanceTensor`` but can store individual fields in separate tensors (Structure of Arrays):
...
.. code-block:: python
    ...
    particles = spy.InstanceList(
        struct=module.Particle.as_struct(),
        data={
            "position": spy.Tensor(device, dtype=module.float3, shape=(10,)),
            "velocity": spy.Tensor(device, dtype=module.float3, shape=(10,))
        }
    )

The calls to ``construct`` and ``update`` will remain identical, however SlangPy will now generate a kernel that reads from and writes to the separate ``position`` and ``velocity`` tensors.

*Note:* Similar to ``InstanceTensor``, an ``InstanceList`` can also be passed as a parameter, and SlangPy will handle kernel generation accordingly.
...
.. code-block:: python
    ...
    class MyParticles(spy.InstanceList):
        ...
        def __init__(self, name: str, count: int):
            ...
            self.name = name
            self.position = spy.Tensor(device, dtype=module.float3, shape=(count,))
            self.velocity = spy.Tensor(device, dtype=module.float3, shape=(count,))
...
If you prefer the simplified ``InstanceTensor``, it can also be inherited in the same way. In this case, Slang fields are ignored, and all attributes are assumed to be Python-only.
...
- Using `InstanceTensor` and `InstanceList` for managing Slang type instances.
...
``InstanceTensor`` especially is a very lightweight wrapper, and can be used interchangeably with normal
``Tensors``. As a result, favoring the use of an ``InstanceTensor`` unless you have a good reason
