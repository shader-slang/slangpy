...
The documentation pipeline has five distinct outputs:
...
candidates; they are not added to the published API automatically.

Generate the legacy API snapshot and run a strict HTML build with:
...
the checked-in snapshot without attempting to import the native extension.
