...
Python can keep exporting names such as `SlangType` and `SlangProgramLayout` for compatibility while the migration is in progress. In the current branch, those names are compatibility aliases for native `sgl::refl::Type` and `sgl::refl::Layout` bindings.

`SlangType` and `SlangProgramLayout` now bind directly to native `sgl::refl::Type` and `sgl::refl::Layout` through the compatibility layer in `slangpy/reflection/reflectiontypes.py`. `NativeSlangType` still exists in the marshalling/runtime binding surface, but it is no longer the semantic reflection source of truth. Phase 6 should replace those marshalling references with `sgl::refl::Type` and then remove `NativeSlangType`.
...
The initial Python binding modules `slangpy.native_refl` and `slangpy.native_func` are also temporary migration names. They make the native/Python boundary explicit while the existing public Python API keeps compatibility names such as `Module.layout` and `Struct.name`. These native classes should not grow a parallel public `native_*` field surface; their data should replace the corresponding Python fields as each native implementation becomes complete.

## Progress So Far

Last checked on 2026-05-18 against the current working tree on `dev/ccummings/native-slangpy`.

Completed:

- Created the working branch `dev/ccummings/native-slangpy`.
- Added the native semantic reflection core:
  ...
  - `sgl::refl::Layout` wraps `sgl::ProgramLayout`, caches types and functions by reflection pointer and name, tracks a hot-reload generation, and updates existing cached objects in place after reload.
  - `sgl::refl::Layout` now provides scalar/vector/matrix/array/tensor helper lookups, tensor view lookups, function and method lookup, overload lookup, argument-type specialization, generic argument parsing, and nested/bool generic argument handling.
  - `sgl::refl::Type::fields()` covers struct fields and synthesized vector swizzle fields.
- Added native Tensor-critical lookup:
  - Added `src/sgl/refl/lookup.h` and `src/sgl/refl/lookup.cpp`.
  - Added `sgl::Device::builtin_layout()` as a device-owned semantic layout cache for the `slangpy` support module.
  - Added native hot-reload and close handling for the built-in layout cache through `sgl::Device` lifetime. Reload/reset remain internal device behavior, not public lookup APIs.
...
- Added the native functional footholds:
  - `src/sgl/func/base_module.h`
  - `src/sgl/func/base_module.cpp`
  - `src/sgl/func/base_struct.h`
  - `src/sgl/func/base_struct.cpp`
  - `sgl::func::BaseModule` stores the composed Slang module and native reflection layout, exposes `device_module`, `layout`, `session`, `device`, and `name` to Python, and refreshes its layout on hot reload.
  - `sgl::func::BaseStruct` stores the owning base module and native `sgl::refl::Type`, exposes struct metadata from native reflection, and preserves the owning composed-layout identity needed by Tensor dtype lookup.
- Added bindings in the new binding folders:
  - `src/slangpy_ext/refl/layout.cpp` binds `sgl::refl::Layout`, `TypeLayout`, all current semantic type classes, tensor/function enums, `Variable`, `Field`, `Parameter`, `Function`, known/unknown helpers, built-in layout lookup, and native layout/type resolution helpers.
  - `src/slangpy_ext/func/functional.cpp` binds `sgl::func::BaseModule` and `sgl::func::BaseStruct`.
- Added transitional Python binding modules and generated stubs:
  - `slangpy.native_refl`
  - `slangpy.native_func`
  - `src/slangpy_ext/CMakeLists.txt` now generates submodule stubs for both modules.
- Retargeted the Python reflection compatibility layer:
  - `slangpy/reflection/reflectiontypes.py` now imports native `slangpy.native_refl` classes and aliases them to existing compatibility names such as `SlangType`, `SlangProgramLayout`, `SlangFunction`, `SlangField`, and `SlangParameter`.
  - The file now mainly keeps Python-side helper tables and functions such as NumPy scalar maps, `is_matching_array_type()`, and `vectorize_type()`.
  - `slangpy/reflection/lookup.py` uses native built-in layout and native layout/type resolution for `SlangType`, `TypeReflection`, `TypeLayoutReflection`, `BaseStruct`, and string inputs, while keeping the Python registry fallback for other Python values.
- Updated Python layering:
  ...
  - `Module.layout`, `Module.name`, `Module.session`, `Module.device`, and `Module.device_module` now come from the native base class.
...
build\windows-msvc\Debug\sgl_tests.exe --test-suite=func --success=false
build\windows-msvc\Debug\sgl_tests.exe --test-suite=refl --success=false
python -m pytest slangpy/tests/slangpy_tests/test_reflection2.py -q
python -m pytest slangpy/tests/slangpy_tests/test_buffer_views.py slangpy/tests/slangpy_tests/test_tensor.py slangpy/tests/slangpy_tests/test_array_return_arrays.py slangpy/tests/slangpy_tests/test_structured_numpy.py -q
python -m pytest slangpy/tests/slangpy_tests/test_type_resolution.py slangpy/tests/slangpy_tests/test_code_gen.py slangpy/tests/slangpy_tests/test_array.py -q
```

Current limitations:

- Native `SlangType`, `SlangProgramLayout`, `SlangFunction`, `SlangField`, and `SlangParameter` are now the active reflection objects, but function marshalling, type resolution orchestration, and call-data generation are still Python/extension-owned.
...
- Public generated docs under `docs/generated` are still stale and contain old `NativeSlangType` references.
- Public compatibility still needs a decision pass:
  - `Module.module` is no longer exposed at runtime; callers must use `device_module` today.
  - `TensorType.Kind` now uses `diff_tensor`, `idiff_tensor`, and `primal_tensor`; old enum spellings such as `difftensor` are not currently accepted.
  - `Layout.tensor_type()` and `TensorType.build_tensor_name()` use the `tensor_kind` keyword; the old `tensor_type` keyword is not currently accepted.
- Native-only discovery of the `slangpy` support module include path is still future work.
...
2. Optional provider fallback.
   `slangpy_ext` can register a provider that calls a Python registry such as the legacy `TYPE_OVERRIDES` mechanism, or a replacement registry with the same user-extensible role.
...
- Implemented for built-in semantic type/function reflection, helper lookups, generic parsing, hot reload, built-in layout caching, and native lookup entry points.
- Remaining Phase 1/2 edge work is compatibility-focused: custom Python metadata fallback, stale public docs/stubs, and old public spellings/keywords.
...
- Mostly implemented for built-in reflection and native lookup. Python compatibility names now alias native bindings, `Module`/`Struct` inherit native bases, and `slangpy/reflection/lookup.py` uses native lookup for native values.
- Not complete for custom Python reflection metadata, public compatibility aliases/keywords, `NativeSlangType` retirement, generated public docs, or final stub cleanup.

Implementation:

- Done: bind `sgl::refl::Type` and derived classes under `src/slangpy_ext/refl`.
- Done: bind `sgl::func::BaseModule` and `sgl::func::BaseStruct` under `src/slangpy_ext/func/functional.cpp`.
- Done: bind transitional Python modules as `slangpy.native_refl` and `slangpy.native_func`; these classes are not added through `slangpy.core.native`.
- Done: make Python `Module` inherit from `BaseModule`.
- Done: make Python `Struct` inherit from `BaseStruct`.
- Done: make Python `SlangType`, `SlangProgramLayout`, `SlangFunction`, `SlangField`, `SlangParameter`, `ITensorType`, and related names compatibility aliases for native bindings.
- Maintained: do not use trampolines for `SlangType`; Python-specific compatibility properties are bound with lambdas or implemented as Python wrapper methods.
- Partially done: binding source lists and generated submodule stubs are wired. `native_func.__init__.pyi`, `py_doc.h`, and generated API docs still need a cleanup/regeneration pass.
- Pending: implement the Python provider fallback for custom Python metadata, or explicitly defer it and update samples/docs that still depend on `TYPE_OVERRIDES`.
- Pending: preserve or intentionally remove old public spellings such as `Module.module`, `TensorType.difftensor`, and the `tensor_type=` keyword for tensor-kind selection.
...
- `src/sgl` must not store `nb::object` or Python-owned compatibility wrappers.
- Legacy Python `TYPE_OVERRIDES` behavior, if preserved, should be adapted by `slangpy_ext` into `CustomTypeDesc` or equivalent native data.
