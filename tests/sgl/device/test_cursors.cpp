...
TEST_CASE("register_cursor_writer_rejects_duplicate_key_kind_and_key")
{
    static int python_type_key;
    ...
    auto make_info = []()
    {
        ...
        info.type_key = &python_type_key;
        info.debug_name = "test.python.Type";
        info.write_shader_cursor = [](ShaderCursor& cursor, const void* value)
        {
            (void)cursor;
            (void)value;
        ...
        {
            (void)value;
            sig.add("sig:python-type");
        };
        return info;
    };

    cursor_utils::register_cursor_writer_type(make_info());
    CHECK_THROWS(cursor_utils::register_cursor_writer_type(make_info()));

    cursor_utils::unregister_cursor_writer_types(cursor_utils::CursorWriterTypeKeyKind::python_type);
}

TEST_CASE("unregister_cursor_writer_types_removes_python_keyed_entries")
{
    static int python_type_key;
    cursor_utils::unregister_cursor_writer_types(cursor_utils::CursorWriterTypeKeyKind::python_type);

    const uint64_t before_register = cursor_utils::cursor_writer_registry_generation();

    cursor_utils::CursorWriterTypeInfo info;
    info.key_kind = cursor_utils::CursorWriterTypeKeyKind::python_type;
    info.type_key = &python_type_key;
    info.debug_name = "test.python.UnregisterType";
    info.write_shader_cursor = [](ShaderCursor& cursor, const void* value)
    {
        (void)cursor;
        (void)value;
    ...
    {
        (void)value;
        sig.add("sig:python-unregister");
    };
    cursor_utils::register_cursor_writer_type(std::move(info));
    CHECK_GT(cursor_utils::cursor_writer_registry_generation(), before_register);

    auto has_python_entry = []()
    {
        for (const auto& entry : cursor_utils::cursor_writer_type_infos()) {
            if (entry.key_kind == cursor_utils::CursorWriterTypeKeyKind::python_type
                && entry.type_key == &python_type_key)
                return true;
        }
        return false;
    };
    CHECK(has_python_entry());

    const uint64_t before_unregister = cursor_utils::cursor_writer_registry_generation();
    cursor_utils::unregister_cursor_writer_types(cursor_utils::CursorWriterTypeKeyKind::python_type);
    CHECK_GT(cursor_utils::cursor_writer_registry_generation(), before_unregister);
    CHECK_FALSE(has_python_entry());

    const uint64_t after_unregister = cursor_utils::cursor_writer_registry_generation();
    cursor_utils::unregister_cursor_writer_types(cursor_utils::CursorWriterTypeKeyKind::python_type);
    CHECK_EQ(cursor_utils::cursor_writer_registry_generation(), after_unregister);
}
