...
TEST_CASE("can_encode_can_decode")
{
    ...
    CHECK_FALSE(codec.can_encode(BCFormat::bc4_snorm));
    CHECK_FALSE(codec.can_encode(BCFormat::bc5_snorm));
...
TEST_CASE("decode_snorm")
{
    ...
    codec.decode(block, sizeof(block), BCFormat::bc4_snorm, 4, 4, int_dst);
    ...
    codec.decode(block, sizeof(block), BCFormat::bc4_snorm, 4, 4, float_dst);
...
TEST_CASE("decode_validates_destination_layout")
{
    ...
    CHECK_THROWS(codec.decode(block, sizeof(block), BCFormat::bc1_unorm, 4, 4, bad_channels));
...
TEST_CASE("software_mipmaps_honor_row_pitch")
{
    ...
    BCCompressedImage tight_result = codec.encode(tight, BCFormat::bc7_unorm, options);
    BCCompressedImage padded_result = codec.encode(padded, BCFormat::bc7_unorm, options);
...
TEST_CASE("software_mipmaps_preserve_constant_color")
{
    ...
    for (BCFormat format : {BCFormat::bc7_unorm, BCFormat::bc7_unorm_srgb}) {
        BCCompressedImage compressed = codec.encode(src, format, options);
        for (const BCCompressedMip& mip : compressed.mip_levels) {
            ...
            codec.decode(mip.data.data(), mip.data.size(), format, mip.width, mip.height, dst);
