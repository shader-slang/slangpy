...
class TestPosRotScale:
    ...
    def test_static_constructors(self):
        ...
        # Translation
        pos = float3(1.0, 2.0, 3.0)
        prs = posrotscalef.translation(pos)
        assert prs.pos == pos
        assert prs.rot == quatf.identity()
        assert prs.scale == float3(1.0, 1.0, 1.0)
        ...
        # Rotation
        rot = quatf(0.0, 0.0, 0.7071067, 0.7071068)
        prs = posrotscalef.from_rotation(rot)
        assert prs.pos == float3(0.0, 0.0, 0.0)
        assert prs.rot == rot
        assert prs.scale == float3(1.0, 1.0, 1.0)
        ...
        # Uniform scaling
        prs = posrotscalef.uniform_scaling(2.5)
        assert prs.pos == float3(0.0, 0.0, 0.0)
        assert prs.rot == quatf.identity()
    ...
    def test_matrix_conversions(self):
        # to_matrix3x4
