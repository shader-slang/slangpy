...
def test_array_of_vector_return(device_type: DeviceType):
    ...
    coord = 5
    # Pre-allocate torch tensor for result: float2[6] = shape (6, 2)
    result = torch.empty((6, 2), dtype=torch.float32, device=torch.device("cuda"))
    module.return_vector_array(coord, _result=result)
    ...
    # Test with float3[4]
    # Pre-allocate torch tensor for result: float3[4] = shape (4, 3)
    result2 = torch.empty((4, 3), dtype=torch.float32, device=torch.device("cuda"))
    module.return_vector_array_float3(coord, _result=result2)
...
def test_array_of_vector_return_with_grid(device_type: DeviceType):
    ...
    # Pre-allocate torch tensor for result: grid(13) x float2[6] = shape (13, 6, 2)
    result = torch.empty((13, 6, 2), dtype=torch.float32, device=torch.device("cuda"))
    module.return_vector_array(coord=spy.grid((13,)), _result=result)
