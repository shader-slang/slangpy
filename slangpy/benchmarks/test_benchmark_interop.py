...
def test_pytorch_tensor_addition_cpu(
...
def test_compute_addition_noalloc_cpu(
    ...
    def tensor_addition(a: spy.NDBuffer, b: spy.NDBuffer, res: spy.NDBuffer):
        kernel.dispatch(spy.uint3(1, 1, BUFFER_SIZE), a=a.storage, b=b.storage, res=res.storage)
...
@pytest.mark.parametrize("device_type", helpers.DEFAULT_DEVICE_TYPES)
def test_pytorch_tensor_addition_gpu_est(device_type: spy.DeviceType, report: ReportFixture):
    device = helpers.get_torch_device(device_type)
...
if __name__ == "__main__":
    ...
    pytest.main([__file__ + "::test_slangpy_addition_noalloc_interop_gpu_est", "-v", "-s"])
