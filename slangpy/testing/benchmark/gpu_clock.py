...
def lock_gpu_clocks(
    ...
    """
    # Validated here rather than only in the CLI, because this is re-exported for
    # benchmark harnesses to call directly. NaN also fails this comparison, which
    # would otherwise reach the selection loop and quietly return None.
    if not 0.0 <= ratio <= 1.0:
        raise ValueError(f"ratio must be between 0.0 and 1.0, got {ratio!r}")
