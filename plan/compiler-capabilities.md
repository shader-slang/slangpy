...
Replace the backend-independent-looking `SlangCompilerOptions.shader_model` with compiler capability selection that actually describes the chosen backend. Users should be able to use device-derived defaults, supply an explicit capability list, adjust individual supplied capabilities, select CUDA compute capabilities, and choose between native D3D12 and NVAPI shader execution reordering (SER). The public API should survive Slang eventually needing fewer profiles; backend adapters may still use profiles internally.
...
For DXIL, capability-only shader-model selection does not reliably set DXC's profile. The effective profile still follows profile fields and defaults. An internal adapter should derive the DX profile from the resolved HLSL version, including known selected capabilities that imply a minimum shader model.

For direct SPIR-V, `slang-target.cpp:114` supplies SPIR-V 1.5 when no version is found in the profile. Merely adding a lower SPIR-V capability cannot undo that baseline. Also, public `spirv_1_5` and `spirv_1_6` aliases include feature bundles absent from the raw `_spirv_*` version atoms (`slang-capabilities.capdef:1655`). Prefer probing a minimal `spirv_1_0` compatibility profile plus the selected raw version/features: this can suppress the hidden 1.5 baseline without importing higher-profile feature bundles. Derive a higher profile only where experiments show it is needed and its implications are intended. Stop using HLSL shader-model profiles to represent Vulkan support.
...
Keep two primary new options, using strings rather than a fixed enum:

    capabilities: list[str] | None = None
    capability_overrides: dict[str, bool] = {}

In C++, represent these as `std::optional<std::vector<std::string>>` and `std::map<std::string, bool>`. Python annotations above describe fields, not a proposed mutable-default Python function signature.

`None` means start from device-derived compiler capability inputs. An explicit list replaces those device-derived inputs. `[]` is distinct from `None`: it requests no optional device-derived inputs. The fixed backend target, mandatory backend baseline, and Slang's implication rules still exist. Document the baseline explicitly; an empty list does not mean a compiler with no capabilities or an invalid device.
...
A version-ceiling convenience operation would be useful, but should be explicit and separate from general boolean semantics. It could remove all higher members of a known version family and reject known dependent capabilities such as `optix_coopvec`. Before adding a third public field/helper, validate whether authoritative lists and version-filtering examples suffice. If the product requirement is instead that every `False` means an absolute prohibition, obtain upstream implication/negative-capability support before claiming that API is implemented.
...
First make the new path explicitly selectable while keeping old defaults for ordinary sessions. Reject simultaneous non-default `shader_model` and new capability selection rather than defining ambiguous precedence. Device/default-option copying and explicitly created sessions must have documented semantics; do not accidentally introduce inheritance from the device's default session into `create_slang_session`, which currently constructs a fresh descriptor.
...
The API must accept device defaults, an authoritative list, and overrides with the exact documented semantics. Explicit unknown names fail early; detected unknown names appear in the resolution report. Empty inputs retain only documented mandatory baselines. Capability lookup must use strings at the boundary and never persisted numeric Slang IDs.
...
2026-09-23, proposal: expose capability strings and optional replacement plus overrides; keep profiles internal. Rationale: this matches current backend diversity while tolerating future Slang profile removal.
...
The research and step-1 probes support a capability-based public API and identify concrete work that a simple field replacement would miss. The proposed API distinguishes supplied assumptions from universal prohibition. Known CUDA tiers work through existing Slang mapping; missing exact tiers require upstream work. DX profile synthesis and minimal SPIR-V baseline profiles are tested candidate adapters. SER strict checking and RHI subgroup reporting are blockers to strict defaults. No production compiler behavior has been changed.
